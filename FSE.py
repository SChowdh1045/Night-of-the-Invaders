...
#song = mixer.music.load('music.mp3')
#mixer.music.play(-1)
...
def outlast():
    ...
    Text_Surf_small, Text_Rect_small =  text_obj_B('YOU OUTNUMBERED THE ENEMIES ',outlast_text_small)
...
def win():
    ...
    Text_Surf_small, Text_Rect_small =  text_obj_B('YOU KILLED ALL THE ENEMIES ',win_text_small)
...
def checkHits(jet,birds, goodX, goodY):
    ...
    for i, enemy_jet in enumerate(jet):
        if ((goodX-enemy_jet[X])**2 + (goodY-enemy_jet[Y])**2)**0.5 < 130:
            enemy_jet[X], enemy_jet[Y] = i*-80,randint(-40,800)   #random set integers for random spawn locations
    ...
    for z, enemy_birds in enumerate(birds):
        if ((goodX-enemy_birds[X])**2 + (goodY-enemy_birds[Y])**2)**0.5 < 40:
            enemy_birds[X], enemy_birds[Y] = z*randint(-20,1030),randint(-20,800) #random set integers for random spawn locations
...
def drawScene(screen, jet, birds, goodGuy, goodAng ):
    ...
    rotate = (goodGuy[X]-main_guy.get_width()//2,goodGuy[Y]-main_guy.get_height()//2)
    ...
    draw.rect(screen,(192,192,192),(460,740,200,15),0) #(The rect itself, not the outline)
...
birds = [  [-200,randint(-30,800),0]  ,  [(width//2),-randint(-30,800),0]  ,  [200,randint(-30,800),0]  ] # inital x,y location for my 3 birds
...
# Menu Image
menu_bg = image.load('menu.jpg')
instructions = image.load('Instructions.png')
Credits = image.load('Credits.png')

#Characters
badPic = image.load("plane.png")
badPic2 = image.load("bird.png")
goodPic = image.load("soldier.png")
backPic = image.load("fire.jpg")

#Bottom part
health = image.load('health.png')
shield = image.load('shield.png')
ammo = image.load('ammo.png')
